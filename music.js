...
(() => {
  ...
  function spotifyEmbedUrl(raw){
    ...
    if(value.startsWith('spotify:')){
      ...
      if(parts.length>=3&&allowed.has(parts[1])&&/^[A-Za-z0-9]+$/.test(parts[2]))return `https://open.spotify.com/embed/${parts[1]}/${parts[2]}`;
      return '';
    }
    try{
      ...
      let typeIndex=parts.findIndex(p=>allowed.has(p));
  ...
  function build(){
    ...
    root.innerHTML=`<div class="blobby-music-backdrop"></div><section class="blobby-music-shell" aria-hidden="true" aria-label="Blobby Music"><header class="blobby-music-head"><div><span>MUSIC</span><h2>Blobby Music</h2><p>Play local audio or use Spotify's official embedded player.</p></div><button class="blobby-music-close" type="button" aria-label="Close Music">×</button></header><div class="blobby-music-source-tabs" role="tablist" aria-label="Music source"><button class="active" type="button" data-source="local" role="tab">Local</button><button type="button" data-source="spotify" role="tab">Spotify</button></div><main class="blobby-music-body" data-source-panel="local"><section class="blobby-music-library"><div class="blobby-music-section-head"><div><small>LOCAL LIBRARY</small><h3>Your queue</h3></div><button class="blobby-music-add" type="button">+ Add music</button></div><input class="blobby-music-picker" type="file" accept="audio/*" multiple hidden><div class="blobby-music-list"></div></section><aside class="blobby-music-player"><div class="blobby-music-art">♫</div><small>NOW PLAYING</small><h3 class="blobby-music-title">Nothing playing</h3><p class="blobby-music-file">Add audio files to begin.</p><div class="blobby-music-progress"><span class="blobby-music-current">0:00</span><input class="blobby-music-seek" type="range" min="0" max="1000" value="0"><span class="blobby-music-duration">0:00</span></div><div class="blobby-music-controls"><button data-act="shuffle" type="button" title="Shuffle">⤨</button><button data-act="prev" type="button" title="Previous">‹</button><button data-act="play" class="primary" type="button" title="Play">▶</button><button data-act="next" type="button" title="Next">›</button><button data-act="repeat" type="button" title="Repeat">↻</button></div><label class="blobby-music-volume"><span>Volume</span><input type="range" min="0" max="100" value="80"></label><p class="blobby-music-note">Local files stay available for this Blobby session. After a full browser/app restart, choose them again.</p></aside></main><main class="blobby-music-spotify" data-source-panel="spotify" hidden><section class="blobby-music-spotify-card"><div class="blobby-music-section-head"><div><small>SPOTIFY EMBED</small><h3>Play from Spotify</h3></div></div><p class="blobby-music-spotify-copy">Paste a Spotify track, album, playlist, artist, show, or episode link. Spotify handles playback inside its official embedded player.</p><div class="blobby-music-spotify-input-row"><input class="blobby-music-spotify-input" type="url" inputmode="url" autocomplete="off" spellcheck="false" placeholder="https://open.spotify.com/playlist/..."><button class="blobby-music-spotify-load" type="button">Load</button></div><div class="blobby-music-spotify-error" role="status" aria-live="polite"></div><div class="blobby-music-spotify-frame-wrap"><div class="blobby-music-spotify-empty">Paste a Spotify link above to load the player.</div></div><div class="blobby-music-spotify-actions"><button class="blobby-music-spotify-clear" type="button">Clear Spotify player</button></div><p class="blobby-music-note">Spotify controls the embedded playback experience. Blobby does not download or store Spotify audio.</p></section></main></section>`;
    ...
    ui={root,shell:root.querySelector('.blobby-music-shell'),backdrop:root.querySelector('.blobby-music-backdrop'),close:root.querySelector('.blobby-music-close'),picker:root.querySelector('.blobby-music-picker'),add:root.querySelector('.blobby-music-add'),list:root.querySelector('.blobby-music-list'),title:root.querySelector('.blobby-music-title'),file:root.querySelector('.blobby-music-file'),seek:root.querySelector('.blobby-music-seek'),current:root.querySelector('.blobby-music-current'),duration:root.querySelector('.blobby-music-duration'),volume:root.querySelector('.blobby-music-volume input'),play:root.querySelector('[data-act="play"]'),shuffle:root.querySelector('[data-act="shuffle"]'),repeat:root.querySelector('[data-act="repeat"]'),mini,miniTitle:mini.querySelector('strong'),miniPlay:mini.querySelector('.blobby-music-mini-play'),sourceTabs:[...root.querySelectorAll('[data-source]')],localPanel:root.querySelector('[data-source-panel="local"]'),spotifyPanel:root.querySelector('[data-source-panel="spotify"]'),spotifyInput:root.querySelector('.blobby-music-spotify-input'),spotifyLoad:root.querySelector('.blobby-music-spotify-load'),spotifyError:root.querySelector('.blobby-music-spotify-error'),spotifyFrameWrap:root.querySelector('.blobby-music-spotify-frame-wrap'),spotifyClear:root.querySelector('.blobby-music-spotify-clear')};
    ui.volume.value=Math.round(audio.volume*100);ui.close.onclick=close;ui.backdrop.onclick=close;ui.add.onclick=()=>ui.picker.click();ui.picker.onchange=e=>addFiles([...e.target.files]);root.querySelector('[data-act="prev"]').onclick=prev;root.querySelector('[data-act="next"]').onclick=next;ui.play.onclick=toggle;ui.shuffle.onclick=()=>{state.shuffle=!state.shuffle;save();renderControls();};ui.repeat.onclick=()=>{state.repeat=state.repeat==='off'?'all':state.repeat==='all'?'one':'off';save();renderControls();};ui.volume.oninput=()=>{audio.volume=+ui.volume.value/100;save();};ui.seek.oninput=()=>{if(Number.isFinite(audio.duration))audio.currentTime=(+ui.seek.value/1000)*audio.duration;};mini.querySelector('.blobby-music-mini-open').onclick=open;mini.querySelector('.blobby-music-mini-play').onclick=toggle;mini.querySelector('.blobby-music-mini-next').onclick=next;
    ...
    audio.ontimeupdate=progress;audio.onloadedmetadata=progress;audio.onplay=renderControls;audio.onpause=renderControls;audio.onended=ended;document.addEventListener('keydown',e=>{if(e.key==='Escape'&&state.open)close();});window.addEventListener('blobby:license-locked',()=>{audio.pause();close();});
    if(state.spotifyUrl){ui.spotifyInput.value=state.spotifyUrl;loadSpotify(state.spotifyUrl,false);}
  ...
  function setSource(source,persist=true){
    ...
    if(state.source==='spotify'){
      ...
      if(state.spotifyUrl&&!ui.spotifyFrameWrap.querySelector('iframe'))loadSpotify(state.spotifyUrl,false);
    }else{
      const frame=ui.spotifyFrameWrap.querySelector('iframe');
      if(frame){frame.src='about:blank';frame.remove();ui.spotifyFrameWrap.replaceChildren(el('div','blobby-music-spotify-empty','Spotify is paused while Local is selected. Switch back to Spotify to reload it.'));}
    }
    const cur=state.queue[state.index];ui.mini.hidden=!cur||state.source!=='local';
    if(persist)save();
  }
  function loadSpotifyFromInput(){loadSpotify(ui.spotifyInput.value,true);}
  function loadSpotify(raw,persist=true){
    const embed=spotifyEmbedUrl(raw);
    if(!embed){ui.spotifyError.textContent='Paste a valid open.spotify.com link or Spotify URI.';return;}
    const original=String(raw||'').trim();
    ui.spotifyError.textContent='';
    ui.spotifyFrameWrap.replaceChildren();
    const frame=document.createElement('iframe');
    frame.className='blobby-music-spotify-frame';frame.src=embed;frame.title='Spotify embedded player';frame.loading='lazy';frame.allow='autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture';frame.setAttribute('allowfullscreen','');frame.setAttribute('frameborder','0');
    ui.spotifyFrameWrap.append(frame);
    state.spotifyUrl=original;
    ui.spotifyInput.value=original;
    if(!audio.paused)audio.pause();
    if(persist)localStorage.setItem('blobby.music.spotify',original);
  }
  function clearSpotify(){
    state.spotifyUrl='';localStorage.removeItem('blobby.music.spotify');ui.spotifyInput.value='';ui.spotifyError.textContent='';ui.spotifyFrameWrap.replaceChildren(el('div','blobby-music-spotify-empty','Paste a Spotify link above to load the player.'));
  }
  ...
  function select(i,autoplay=true){if(!state.queue.length)return;i=(i+state.queue.length)%state.queue.length;state.index=i;audio.src=state.queue[i].url;audio.load();render();if(autoplay){setSource('local');audio.play().catch(()=>{});}}
  function toggle(){if(state.index<0){if(state.queue.length)select(0,true);return;}setSource('local');if(audio.paused)audio.play().catch(()=>{});else audio.pause();}
  ...
  window.BlobbyMusic=Object.freeze({open,close,toggle:()=>state.open?close():open(),state:()=>({open:state.open,playing:!!audio&&!audio.paused,index:state.index,count:state.queue.length,source:state.source,spotifyLoaded:!!state.spotifyUrl})});
})();
